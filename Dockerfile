...
RUN apt-get -y install wget php php-fpm php-mbstring php-mysql php-gd nginx lsb-release gnupg
...
RUN	tar xzf /tmp/wordpress.tar.gz -C /var/www/html && \
	...
	service mysql start && \
	wp core install --allow-root --url=localhost --path=/var/www/html/ --title=ft_server --admin_user=root --admin_password=wachtwoord --admin_email=novan-ve@student.codam.nl --skip-email && \
	service mysql stop && \
